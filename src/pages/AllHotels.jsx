...
function AllHotels() {
  ...
  return (
    <Layout>
      <div
        className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex flex-col-reverse lg:flex-row items-start justify-between px-4 md:px-6
      ...
      >
        <div>
          ...
          {filteredAndSortedHotels.map((room) => (
            <div
              key={room.id}
              className="w-[58rem] bg-white rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 p-6 mb-6 border border-gray-100 hover:border-indigo-200"
            >
              <div className="flex flex-col md:flex-row items-start gap-6">
                ...
                <div className="md:w-1/2 flex flex-col gap-4">
                  <p className="text-gray-800 text-2xl md:text-3xl font-playfair cursor-pointer hover:text-indigo-600 transition-colors">
                    {room.name}
                  </p>
                  ...
                  <div className="flex flex-wrap items-center gap-3 mt-2">
                    {room.facilities.map((item, index) => (
                      <div
                        key={index}
                        className="flex items-center gap-2 px-3 py-2 rounded-full bg-gray border border-customBlue hover:bg-safari-gold transition-colors"
                      >
                        <img
                          src={item.icon}
                          alt={item.name}
                          className="w-4 h-4"
                        />
                        <p className="text-xs text-safari-dark font-medium">
                          {item.name}
                        </p>
                      </div>
                    ))}
        ...
        {/*filters*/}
        <div
          className={`bg-white w-[15rem] border-2 border-gray-300 text-gray-600 max-lg:mb-8 min-lg:mt-16`}
        >
          <div
          ...
          >
            <div className="px-6 pt-6">
              <p className="font-semibold text-gray-800 pb-4 text-customBlue">
                Popular filters
              </p>
